...
NvidiaH264EncoderImpl::NvidiaH264EncoderImpl(
  ...
  // Map H264 profile to NVENC profile GUID
  switch (profile_) {
  ...
  RTC_LOG(LS_INFO) << "NVENC H264 profile: " << static_cast<int>(profile_)
                   << " level: " << static_cast<int>(level_)
                   << " profile-level-id: " << hexString;
...
int32_t NvidiaH264EncoderImpl::InitEncode(
  ...
  try {
    ...
    std::cout << "[NVENC] CreateEncoder FAILED: " << e.what() << std::endl;
    return WEBRTC_VIDEO_CODEC_ERROR;
  }

  std::cout << "[NVENC] InitEncode OK: " << codec_.width << "x" << codec_.height
            << " @ " << codec_.maxFramerate << "fps"
            << " startBitrate=" << codec_.startBitrate << "kbps"
            << " maxBitrate=" << codec_.maxBitrate << "kbps"
            << " target_bps=" << configuration_.target_bps
            << " max_bps=" << configuration_.max_bps
            << " frameDrop=" << configuration_.frame_dropping_on
            << " simulcastStreams=" << codec_.numberOfSimulcastStreams
            << std::endl;
...
int32_t NvidiaH264EncoderImpl::Encode(
  ...
  static int encode_call_count_ = 0;
  static int encode_success_count_ = 0;
  static int encode_skip_count_ = 0;
  ...
  try {
    ...
    encode_success_count_++;
  } catch (const NVENCException& e) {
    std::cout << "[NVENC] EncodeFrame EXCEPTION: " << e.what() << std::endl;
    return WEBRTC_VIDEO_CODEC_ENCODER_FAILURE;
  }
  ...
  if (elapsed >= 3) {
    std::cout << "[NVENC] Encode stats: calls=" << encode_call_count_
              << " encoded=" << encode_success_count_
              << " skipped=" << encode_skip_count_
              << " target_bps=" << configuration_.target_bps
              << " sending=" << configuration_.sending
              << std::endl;
    last_log_time_ = now;
  }
...
void NvidiaH264EncoderImpl::SetRates(
  ...
  static int setrates_count_ = 0;
  setrates_count_++;
  if (setrates_count_ <= 5 || setrates_count_ % 30 == 0) {
    std::cout << "[NVENC] SetRates #" << setrates_count_
              << ": bitrate=" << parameters.bitrate.get_sum_bps() / 1000 << "kbps"
              << " fps=" << parameters.framerate_fps
              << " spatial0=" << parameters.bitrate.GetSpatialLayerSum(0) / 1000 << "kbps"
              << std::endl;
  }
  ...
  if (new_target_bps != configuration_.target_bps) {
    ...
    try {
      ...
      std::cout << "[NVENC] Reconfigure FAILED: " << e.what() << std::endl;
    }
  }
