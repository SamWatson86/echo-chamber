...
#[cfg(windows)]
fn lock_unpoisoned<'a, T>(mutex: &'a Mutex<T>, context: &str) -> std::sync::MutexGuard<'a, T> {
  match mutex.lock() {
    Ok(guard) => guard,
    Err(poisoned) => {
      eprintln!(
        "[tauri-runtime-wry] recovered poisoned mutex in {}",
        context
      );
      poisoned.into_inner()
    }
...
#[cfg(windows)]
fn swallow_noncritical_callback_panic(
  callback_name: &str,
  f: impl FnOnce() -> windows::core::Result<()>,
) -> windows::core::Result<()> {
  match panic::catch_unwind(panic::AssertUnwindSafe(f)) {
    Ok(result) => result,
    Err(_) => {
      eprintln!(
        "[tauri-runtime-wry] swallowed panic in {} callback",
        callback_name
      );
      Ok(())
    }
      ...
      TaoWindowEvent::Focused(focused) => {
        ...
        if window.has_children.load(Ordering::Relaxed) {
          ...
          let mut focused_webview =
            lock_unpoisoned(&window.focused_webview, "WindowEventWrapper::Focused");
  ...
  #[cfg(windows)]
  {
    ...
    unsafe {
      ...
      controller.add_GotFocus(
        &FocusChangedEventHandler::create(Box::new(move |_, _| {
          swallow_noncritical_callback_panic("webview-got-focus", || {
            let mut focused_webview =
              lock_unpoisoned(&focused_webview_, "webview-got-focus::focused_webview");
            ...
            if !already_focused {
              let _ = proxy_clone.send_event(Message::Webview(
                *lock_unpoisoned(&window_id_, "webview-got-focus::window_id"),
                id,
    ...
    unsafe {
      ...
      controller.add_LostFocus(
        &FocusChangedEventHandler::create(Box::new(move |_, _| {
          swallow_noncritical_callback_panic("webview-lost-focus", || {
            let mut focused_webview =
              lock_unpoisoned(&focused_webview, "webview-lost-focus::focused_webview");
            ...
            if lost_window_focus {
              ...
              *focused_webview = None;
              let _ = proxy_clone.send_event(Message::Webview(
                *lock_unpoisoned(&window_id_, "webview-lost-focus::window_id"),
                id,
      ...
      unsafe {
        let _ = webview.add_ContainsFullScreenElementChanged(
          &ContainsFullScreenElementChangedEventHandler::create(Box::new(move |sender, _| {
            swallow_noncritical_callback_panic("webview-fullscreen-changed", || {
              ...
              let _ = proxy_clone.send_event(Message::Window(
                *lock_unpoisoned(&window_id, "webview-fullscreen-changed::window_id"),
