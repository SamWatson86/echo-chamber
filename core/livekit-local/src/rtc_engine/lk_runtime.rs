...
lazy_static! {
    ...
    // Keep the factory alive forever so NVENC detection at startup persists.
    // Without this, the Weak drops when no Room is alive, and re-creation
    // during gaming fails CUDA check → falls back to OpenH264 (CPU, 9fps).
    static ref LK_RUNTIME_KEEP_ALIVE: Mutex<Option<Arc<LkRuntime>>> = Mutex::new(None);
}
    ...
    pub fn instance() -> Arc<LkRuntime> {
        ...
        if let Some(lk_runtime) = lk_runtime_ref.upgrade() {
            ...
            *lk_runtime_ref = Arc::downgrade(&new_runtime);
            // Pin the runtime so it survives Room drops — preserves NVENC factory
            *LK_RUNTIME_KEEP_ALIVE.lock() = Some(new_runtime.clone());
            new_runtime
        }
    }
